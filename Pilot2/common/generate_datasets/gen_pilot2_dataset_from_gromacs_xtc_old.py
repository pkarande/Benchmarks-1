...
def partition_work(mdsys):
    ...
    totalchunks = 0
    totcheck = float(5.5) / float(fchunksize)
...
#    outA = np.zeros([fchunksize, len(frags), 12], dtype={'names':names, 'formats':['float']*len(names)})
    ...
    outNbrs = -1*np.ones([fchunksize, len(frags), len(frags)])
    ...
    for curframe in range(1):#first_frame, last_frame):
        j = last_frame - curframe
        #frame_ind = curframe - first_frame
        print "Curr frame: ", curframe, "\n"
        mdt[curframe]
        ...
        for curfrag in range(len(frags)):
            ...
            if not curfrag%200:
                print "Current molecule started: ", curfrag
            ...
            if not curfrag%200:
                print "Current molecule ended: ", curfrag, "\n"

            # Extrac t and save meighbors ordered by distance
            if fr.resnum in tp.resnums:
                ind = np.argwhere(tp.resnums==fr.resnum)
                _, klist = top_nbrs.get_nbrs_k(ind[0,0], num_top, True)
                current_resnums = tp[klist].resnums
                outNbrs[i,curfrag,:num_top] = [resnum_dict.get(x, None) for x in current_resnums]
            elif fr.resnum in bt.resnums:
                ind = np.argwhere(bt.resnums==fr.resnum)
                _, klist = bot_nbrs.get_nbrs_k(ind[0,0], num_bot, True)
                current_resnums = bt[klist].resnums
                outNbrs[i,curfrag,:num_bot] = [resnum_dict.get(x, None) for x in current_resnums]

            if not curfrag%200:
                print "Current molecule neighborhood ended: ", curfrag, "\n"
...
def flush_chunk_to_file(processname, i, outA, outNbrs, outResnums, outfile, chunkcount, totalchunks):
    ...
    np.savez_compressed(myfilename, features=outA, neighbors=outNbrs, resnums=outResnums) # if this is slow can also do uncompressed "savez"
