...
##### Read Data ########
    ...
    import helper
    (data_files, fields) = helper.get_local_files('/p/gscratchr/brainusr/datasets/cancer/pilot2/3k_run10_10us.35fs-DPPC.10-DOPC.70-CHOL.20.dir/')
    ...
    if not GP['type_bool']:
        ...
        mol_kernel_size = (X.shape[3]-5)*X.shape[2]
    else:
        ...
        mol_kernel_size = X.shape[3]*X.shape[2]
...
######## Define Molecular Model, Solver and Compile #########
    ...
    if conv_bool:
        molecular_model = hf.conv_dense_mol_auto(GP['mol_filters'], mol_k_size=mol_kernel_size,
                                                 weights_path=None, input_shape=(1, molecular_input_dim),
